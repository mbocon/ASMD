		...
		<div>
			...
			<div className='range-grid'>
				{showForm === false ? (
					<Fragment>
						<button
							className={showBtn === false ? 'display btn btn-secondary range-btn' : 'hidden btn btn-secondary'}
							onClick={e => toggleRangeBtn(e)}>
							Select Range
						</button>
						<button
							className={showBtn === false ? 'hidden btn btn-secondary' : 'display btn btn-secondary range-btn'}
			...
			<div className='add'>
            <h3>Multiplication flashcards</h3>
            {range.length ? (
                <div>
			...
			<div className='grid'>
				{gridArr.map(i => {
					return (
						<Fragment key={i}>
							{theIndex !== i ? (
								<div className='grid-box' style={{ backgroundColor: '#F3969A' }} onClick={e => toggleAns(e, i)}>
									<span>
										{range[i]} x {range[i + 1]}
									</span>
								</div>
