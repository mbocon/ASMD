				...
				<div className='grid'>
					{gridArr.map(i => {
						return (
							<Fragment key={i}>
								{theIndex !== i ? (
									<div className='grid-box' style={{ backgroundColor: '#ffce67' }} onClick={e => toggleAns(e, i)}>
										<span>
											{range[i]} / {range[i + 1]}
										</span>
									</div>
									...
									<span className={i === theIndex ? 'showAns' : 'hidden'} onClick={e => toggleAns(e, i)}>
										{range[i] / range[i + 1] === Infinity ? range[i] : range[i] / range[i + 1]}
									</span>
								)}
							</Fragment>
						);
